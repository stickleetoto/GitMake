...
func aiManifest() AIManifest {
	return AIManifest{
		...
		Capabilities: []AICapability{
			...
			{Name: "prepare", Description: "High-level MCP workflow for ZIP-only projects: discover source, infer/validate config, run security/preflight/identity checks, and create a reviewed plan in one tool call. Read-only MCP keeps missing config in memory; write-enabled MCP can persist it atomically.", Mutating: false},
		...
		Commands: map[string]AICommand{
			...
			"mcp_prepare":      {Command: "MCP tool: gitmake_prepare", Description: "Preferred one-call ZIP-to-reviewed-plan workflow. Never use host filesystem Write/Edit to create gitmake.json when this tool is available.", Mutating: false},
		...
		Workflow: []string{
			...
			"When the MCP tool `gitmake_prepare` is available, prefer it for ZIP-only or unconfigured projects. It owns discovery, config inference/validation, preflight, identity checks, and reviewed plan creation. Do not create or edit gitmake.json with host filesystem Write/Edit tools when `gitmake_prepare` or `gitmake_config_write` is available.",
