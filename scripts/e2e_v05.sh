...
assert x['name']=='gitmake' and x['version'].startswith('1.'), x
...
x=json.load(open(sys.argv[1],encoding='utf-8'))
assert x['ok'] is False
assert 'no project source found' in x['error']['message']
PY
test ! -e "$R/gitmake.json"
...
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8'))
assert x['schema']=='gitmake.version/v1' and x['name']=='gitmake', x
parts=x['version'].split('.')
assert len(parts)==3 and parts[0]=='1' and all(part.isdigit() for part in parts), x
PY
